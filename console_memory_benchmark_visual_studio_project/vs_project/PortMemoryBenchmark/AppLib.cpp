...
BOOL AppLib::buildExecPath(LPSTR& dst, size_t max, LPCSTR src)
{
	memset(dst, 0, max);  // blank buffer
	int n = GetModuleFileName(NULL, dst, (DWORD)max);  // get this executable file path by WinAPI
	int i;
	LPSTR p = dst + n - 1;
	...
	p++;
	while (*p++ = *src++);  // copy report file name instead just blanked executable file name
	return TRUE;
}
...
{
	DWORD32 xLow = (DWORD32)x;
	DWORD32 xHigh = (DWORD32)((x >> 16) >> 16);
	int count = 0;
	if (h)
	{
...
void AppLib::printPointer(LPSTR& dst, size_t& max, LPVOID x, BOOL h)
{
	DWORD64 y = (DWORD64)x;
#if NATIVE_WIDTH == 32
	AppLib::print32(dst, max, (DWORD32)y, h);
#endif
#if NATIVE_WIDTH == 64
